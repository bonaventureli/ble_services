...
#if defined(INGEEK_AES_ROM_TABLES)
/*
 * Forward S-box
 */
...
#endif /* INGEEK_AES_ROM_TABLES */

void ingeek_aes_init( ingeek_aes_context *ctx )
{
    memset( ctx, 0, sizeof( ingeek_aes_context ) );
}
...
void ingeek_aes_free( ingeek_aes_context *ctx )
{
    ...
    ingeek_zeroize( ctx, sizeof( ingeek_aes_context ) );
}

/*
 * AES key schedule (encryption)
 */
#if !defined(INGEEK_AES_SETKEY_ENC_ALT)
                    ...
                    unsigned int keybits )
{
    ...
    switch( keybits )
    {
        ...
        default : return( INGEEK_ERR_AES_INVALID_KEY_LENGTH );
    }
...
#endif /* !INGEEK_AES_SETKEY_ENC_ALT */

/*
 * AES key schedule (decryption)
 */
#if !defined(INGEEK_AES_SETKEY_DEC_ALT)
int ingeek_aes_setkey_dec( ingeek_aes_context *ctx, const unsigned char *key,
                    unsigned int keybits )
{
    int i, j, ret;
    ingeek_aes_context cty;
    uint32_t *RK;
    uint32_t *SK;
    ...
    /* Also checks keybits */
    if( ( ret = ingeek_aes_setkey_enc( &cty, key, keybits ) ) != 0 )
        goto exit;
...
#endif /* !INGEEK_AES_SETKEY_DEC_ALT */
 ...
 */
#if !defined(INGEEK_AES_ENCRYPT_ALT)
int ingeek_internal_aes_encrypt( ingeek_aes_context *ctx,
...
#endif /* !INGEEK_AES_ENCRYPT_ALT */

void ingeek_aes_encrypt( ingeek_aes_context *ctx,
                          const unsigned char input[16],
                          unsigned char output[16] )
{
    ingeek_internal_aes_encrypt( ctx, input, output );
}

/*
 * AES-ECB block decryption
 */
#if !defined(INGEEK_AES_DECRYPT_ALT)
int ingeek_internal_aes_decrypt( ingeek_aes_context *ctx,
                                  const unsigned char input[16],
                                  unsigned char output[16] )
{
    int i;
...
#endif /* !INGEEK_AES_DECRYPT_ALT */

void ingeek_aes_decrypt( ingeek_aes_context *ctx,
                          const unsigned char input[16],
                          unsigned char output[16] )
{
    ingeek_internal_aes_decrypt( ctx, input, output );
}
...
int ingeek_aes_crypt_ecb( ingeek_aes_context *ctx,
                    ...
                    unsigned char output[16] )
{
    if( mode == INGEEK_AES_ENCRYPT )
        return( ingeek_internal_aes_encrypt( ctx, input, output ) );
    else
        return( ingeek_internal_aes_decrypt( ctx, input, output ) );
}
                    ...
                    unsigned char *output )
{
    ...
    if( length % 16 )
        return( INGEEK_ERR_AES_INVALID_INPUT_LENGTH );

    if( mode == INGEEK_AES_DECRYPT )
    {
        while( length > 0 )
        {
            memcpy( temp, input, 16 );
            ingeek_aes_crypt_ecb( ctx, mode, input, output );
    ...
    else
    {
        while( length > 0 )
        {
            ...
            ingeek_aes_crypt_ecb( ctx, mode, output, output );
            memcpy( iv, output, 16 );
